anteBayesA <-function(y,Z,startpi,startdf,startscale,truepi,truedef,truescale,truet, numiter,skip,burnIn,Seed=1003,Save.at="results")  
...
{   # START OF FUNCTION
	  ...
	  #Save.at=paste("Change to your directory",sep=" ")
	  # Parameters
	  ...
	  {  #"START" OF MCMC SAMPLING
	     ...
	     if (itersave==1) {Mlast = theta;SNPvarlast=varq;Tlast= assoct;} 
	     ...
	     if (!truescale) 
	      {  #"START" OF SAMPLE THE SCALE PARAMETER 
	           shape_g = 0.5*(nSNP*def+1)
	           scale_g = 0.5*def*sum(1/varq)
